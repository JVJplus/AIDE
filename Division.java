...
public class Division
{
	...
	public void draw(BigInteger n1,BigInteger n2)
	{
		...
		while(productStart<quotient.length())
		{
			...
			{
				if((diff+"").length()>1)
					upperDigits=(n1.toString()).substring(substringStart,substringStart+((currentProduct+"").length()-(diff+"").length()));
				else
			...
			{
				//upperDigits=n1.toString().substring(((n1.toString().length())-((currentProduct+"").length())));
			}

			if(upperDigits.equals(""))
			//upperDigits=n1.toString().substring(((n1.toString().length())-((currentProduct+"").length())));
				upperDigits=""+currentProduct;
			//System.out.println("up="+upperDigits);

			diff=diff*10+Integer.parseInt(upperDigits)-currentProduct;
			for(i=1;i<=currentSpace;i++)
				System.out.print(" ");
